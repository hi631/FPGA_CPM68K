...
void savecpm(){
    ...
    dm=write(fd, &g_ram[0xfe0000L], 32767); dm=dm;
...
void sdcd_rw512(unsigned int cmd, unsigned int sector){
  ...
  else fprintf(stderr,"%c.Err %d", mode,i);
 ...
 */
int trace_disp(int pc){
  char buf[256];
  int rc;

  rc = m68k_disassemble(buf, pc, M68K_CPU_TYPE_68000);
  strcat(buf, "                                "); buf[28] = 0;
  fprintf(stderr, "%06x %s D0:%08x D1:%08x A0:%08x A1:%08x A7:%08x\r\n", pc, buf,
	  m68k_get_reg(NULL, M68K_REG_D0),	  m68k_get_reg(NULL, M68K_REG_D1),
	  m68k_get_reg(NULL, M68K_REG_A0),	  m68k_get_reg(NULL, M68K_REG_A1),
	  m68k_get_reg(NULL, M68K_REG_A7));
  //if(pc > 0x8000) termination_handler(0);
  return rc;
}
...
void running(int rnum){
  ...
  while(g_runf==1){
    ...
    if(pc==rnum) g_runf = 0; // addr  break Stop
    // asm(" dc.w $6004,$4e72,$0000,$4e71"); /* bra *+4; stop; nop;nop; */
    wp = (unsigned int *)&g_ram[pc]; if(*wp==0x724e0460) g_runf = 0; // Break
    if(bwct++==1000) {
      bwct = 0; 
      //if(keybreak()==1) g_runf = 0;
      if(keysave==0x1b) { g_runf = 0; keysave = 0;}
      output_device_update(); input_device_update(); nmi_device_update();
    }
  }
  fprintf(stderr,"\n\r");
  trace_disp( m68k_get_reg(NULL, M68K_REG_PC));
}
//***************************************
// The main loop
//***************************************
int main(int argc, char* argv[]){
  ...
  int adr,ade,adro,dt,dtr,wk;
  char cbuf[128],obuf[128];
  int rnum,cmdx,rnn,rno,mmd;
  unsigned short *wrp;
  char cmd, *cbp, **cbpp;
    ...
    switch(cmd){
      case 'R':
        switch(cbuf[1]){
          ...
          default :
            printf("PC=%06x\n\r", m68k_get_reg(NULL, M68K_REG_PC));
            for( rnn=0; rnn<2; rnn++){
              for( rno=0; rno<8; rno++){
                if(rnn==0) printf("D%d:%08x ", rno, m68k_get_reg(NULL, M68K_REG_D0+rno));
                else       printf("A%d:%08x ", rno, m68k_get_reg(NULL, M68K_REG_A0+rno));
              }
              printf("\n\r");
            }
            break;
      ...
      case 'W' :
        if(cbuf[1]=='W') savecpm(); // save 0-7fff -> image
		break;
